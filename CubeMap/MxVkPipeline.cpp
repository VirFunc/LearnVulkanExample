...
namespace Mix {
    namespace Vk {
        ...
        Pipeline & Pipeline::operator=(Pipeline && pipeline) {
            this->mIsReady = pipeline.mIsReady;
            this->mManager = pipeline.mManager;
            ...
            pipeline.mIsReady = false;
            pipeline.mManager = nullptr;
        ...
        void Pipeline::addShader(const VkShaderStageFlagBits stage, const VkShaderModule shader, const VkSpecializationInfo* specInfo) {
        ...
        bool PipelineManager::setup(const Manager * manager) {
            if (mIsReady)
                destroy();
        ...
        Pipeline & PipelineManager::createPipeline(const std::string & name, const VkRenderPass renderPass, const uint32_t subpassIndex) {
            if (mPipelines.count(name) != 0)
                throw std::runtime_error("Error : [" + name + "] already exists");

            Pipeline pipeline;
            pipeline.setup(mManager);
            pipeline.setTargetRenderPass(renderPass, subpassIndex);
            mPipelines[name] = std::move(pipeline);
            return mPipelines[name];
        }
        const Pipeline & PipelineManager::getPipeline(const std::string & name) {
            if (mPipelines.count(name) == 0)
