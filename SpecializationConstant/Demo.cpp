        ...
        if (seconds > FIXED_UPDATE_TIMESTEP) {
            ...
            while (SDL_PollEvent(&event)) {
                ...
                case SDL_KEYDOWN:
                    ...
                    case SDL_SCANCODE_W:
                        mCamera.moveCamera(seconds, Mix::Camera::FORWARD);
                        break;
                    case SDL_SCANCODE_S:
                        mCamera.moveCamera(seconds, Mix::Camera::BACK);
                        break;
                    case SDL_SCANCODE_A:
                        mCamera.moveCamera(seconds, Mix::Camera::LEFT);
                        break;
                    case SDL_SCANCODE_D:
                        mCamera.moveCamera(seconds, Mix::Camera::RIGHT);
                        break;
                    case SDL_SCANCODE_SPACE:
                        mCamera.moveCamera(seconds, Mix::Camera::WORLD_UP);
                        break;
                    case SDL_SCANCODE_LCTRL:
                        mCamera.moveCamera(seconds, Mix::Camera::WORLD_DOWN);
                        break;
                    }
                }
            }
...
bool TestDemo::init() {
    ...
    mCamera.setPerspective(45.0f, 600.0f / 480.0f, 0.1f, 100.0f);
...
#pragma region specialization_constant


        //preparing specialization constant
        struct SpecializationData {
            float r; float g; float b;
        }specializationData;

        specializationData.r = 0.3f;
        specializationData.g = 0.3f;
        specializationData.b = 0.4f;

        //each shader constant of a shader stage corresponds to one map entry
        std::array<VkSpecializationMapEntry, 3> specMapEntries;
        specMapEntries[0].constantID = 0;
        specMapEntries[0].size = sizeof(specializationData.r);
        specMapEntries[0].offset = offsetof(SpecializationData, r);

        specMapEntries[1].constantID = 1;
        specMapEntries[1].size = sizeof(specializationData.g);
        specMapEntries[1].offset = offsetof(SpecializationData, g);

        specMapEntries[2].constantID = 2;
        specMapEntries[2].size = sizeof(specializationData.b);
        specMapEntries[2].offset = offsetof(SpecializationData, b);

        //preparing specialization info
        ...
        mPipeline->addShader(fragShader->stage, fragShader->module, &specInfo);
