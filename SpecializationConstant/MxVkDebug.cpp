...
namespace Mix {
    namespace Vk {
        ...
        Debug::Debug() :mCreateDebugUtilsMessenger(VK_NULL_HANDLE),
            mDestroyDebugUtilsMessenger(VK_NULL_HANDLE) {
        }

        bool Debug::setup(const Manager* manager) {
            if (mIsReady)
                destroy();
            mManager = manager;
        ...
        bool Debug::setDebugCallback(Severity severity, Type type, PFN_vkDebugUtilsMessengerCallbackEXT callback, void * userData) {
            if (!mIsReady)
                return false;
        ...
        bool Debug::setDefaultCallback(Severity severity, Type type) {
            if (!mIsReady)
                return false;
